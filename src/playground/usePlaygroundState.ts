...
export function usePlaygroundState(initial: PlaygroundProject) {
  ...
  const removeChildPairing = useCallback((parentNodeId: string, edgeId: string) => {
    setProject(prev => {
      const edge = prev.nodes[parentNodeId]?.childEdges.find(e => e.id === edgeId);
      if (!edge) return prev;
      ...
      const updatedParent: PlaygroundNode = {
        ...prev.nodes[parentNodeId],
        childEdges: prev.nodes[parentNodeId].childEdges.filter(e => e.id !== edgeId),
      };
  ...
  const renameOutcome = useCallback((nodeId: string, gKey: string, alias: string | null) => {
    setProject(prev => {
      const node = prev.nodes[nodeId];
      if (!node) return prev;
  ...
  return { project, setProject, addChildPairing, removeChildPairing, renameOutcome };
}
