...
export function PairingsPage() {
  const { pairings, removePairing, updatePairingNotes, animals, saveProject } =
    useAppContext()
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {pairings.map((pairing) => {
                const animal1 = animals.find(
                  (a) => a.id === pairing.parent1AnimalId
                )
                const animal2 = animals.find(
                  (a) => a.id === pairing.parent2AnimalId
                )
                return (
                  <tr
                    key={pairing.id}
