...
const MODIFIER_KEYWORDS = new Set(['het', 'dbl', 'triple', 'quad', 'super', 'pos']);

/**
 * Try to match a gene starting at `words[startIdx]`.
 * Returns [wordCount, geneId] on success or [0, null] on failure.
 */
function tryMatchGene(words: string[], startIdx: number): [number, string | null] {
 ...
 */
function tryMatchCombo(words: string[], startIdx: number): [number, string[] | null] {
